...
export default class PriceCommand extends BaseCommand {
  ...
  async run(payload: Context, args: CommandArgs): Promise<any> {
      ...
      embed.setTitle(`${finalResult[0].name} | ${finalResult[0].symbol}`)
        ...
        .addField('ATH', `USD$${finalResult[0].quotes.USD.ath_price} (${athDate.getFullYear()}-${("0" + (athDate.getMonth() + 1)).slice(-2)}-${("0" + (athDate.getUTCDate())).slice(-2)} ${athDate.getHours()}:${(athDate.getMinutes()<10?'0':'') + athDate.getMinutes()} EST)`, true)
        ...
        .setFooter(`Data provided by Coinpaprika. Last updated at ${updated.getFullYear()}-${("0" + (updated.getMonth() + 1)).slice(-2)}-${("0" + (updated.getUTCDate())).slice(-2)} ${updated.getHours()}:${(updated.getMinutes()<10?'0':'') + updated.getMinutes()} EST`)
