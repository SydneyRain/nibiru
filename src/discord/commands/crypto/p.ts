...
export default class PriceCommand extends BaseCommand {
  constructor(client: CommandClient) {
    super(client, {
      ...
      metadata: {
        description: 'Look up a cryptocurrency\'s current price. Currently supports ~61 different coins (type n.p help for a full list)',
  ...
  async run(payload: Context, args: CommandArgs): Promise<any> {
      ...
      embed.setTitle(`${finalResult[0].name} | ${finalResult[0].symbol}`)
        ...
        .addField('ATH', `USD$${finalResult[0].quotes.USD.ath_price}`, true)
        ...
        .setFooter(`Data provided by Coinpaprika. Last updated at ${updated.getDay()}-${updated.getMonth()}-${updated.getFullYear()} ${updated.getHours()}:${(updated.getMinutes()<10?'0':'') + updated.getMinutes()} UTC`)
